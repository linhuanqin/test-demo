...
export default {
    ...
    format: function(dateTime, pattern) {
        ...
        return pattern.replace(/([a-z])\1*/ig, function(matchStr, group1) {
            var replacement = "";
            switch(group1) {
                case _this.patterns.PATTERN_ERA: //G
                    break;
                case _this.patterns.PATTERN_WEEK_YEAR: //Y
                case _this.patterns.PATTERN_YEAR: //y
                    replacement = date.getFullYear();
                    break;
                case _this.patterns.PATTERN_MONTH: //M
                    var month = date.getMonth() + 1;
                    replacement = (month < 10 && matchStr.length >= 2) ? "0" + month : month;
                    break;
                case _this.patterns.PATTERN_DAY_OF_MONTH: //d
                    var days = date.getDate();
                    replacement = (days < 10 && matchStr.length >= 2) ? "0" + days : days;
                    break;
                case _this.patterns.PATTERN_HOUR_OF_DAY1: //k(1~24)
                    var hours24 = date.getHours();
                    replacement = hours24;
                    break;
                case _this.patterns.PATTERN_HOUR_OF_DAY0: //H(0~23)
                    var hours24 = date.getHours();
                    replacement = (hours24 < 10 && matchStr.length >= 2) ? "0" + hours24 : hours24;
                    break;
                case _this.patterns.PATTERN_MINUTE: //m
                    var minutes = date.getMinutes();
                    replacement = (minutes < 10 && matchStr.length >= 2) ? "0" + minutes : minutes;
                    break;
                case _this.patterns.PATTERN_SECOND: //s
                    var seconds = date.getSeconds();
                    replacement = (seconds < 10 && matchStr.length >= 2) ? "0" + seconds : seconds;
                    break;
                case _this.patterns.PATTERN_MILLISECOND: //S
                    var milliSeconds = date.getMilliseconds();
                    replacement = milliSeconds;
                    break;
                case _this.patterns.PATTERN_DAY_OF_WEEK: //E
                    var day = date.getDay();
                    replacement = _this.week['ch'][day];
                    break;
                case _this.patterns.PATTERN_DAY_OF_YEAR: //D
                    replacement = _this.dayOfTheYear(date);
                    break;
                case _this.patterns.PATTERN_DAY_OF_WEEK_IN_MONTH: //F
                    var days = date.getDate();
                    replacement = Math.floor(days / 7);
                    break;
                case _this.patterns.PATTERN_WEEK_OF_YEAR: //w
                    var days = _this.dayOfTheYear(date);
                    replacement = Math.ceil(days / 7);
                    break;
                case _this.patterns.PATTERN_WEEK_OF_MONTH: //W
                    var days = date.getDate();
                    replacement = Math.ceil(days / 7);
                    break;
                case _this.patterns.PATTERN_AM_PM: //a
                    var hours24 = date.getHours();
                    replacement = hours24 < 12 ? "\u4e0a\u5348" : "\u4e0b\u5348";
                    break;
                case _this.patterns.PATTERN_HOUR1: //h(1~12)
                    var hours12 = date.getHours() % 12 || 12; //0转为12
                    replacement = (hours12 < 10 && matchStr.length >= 2) ? "0" + hours12 : hours12;
                    break;
                case _this.patterns.PATTERN_HOUR0: //K(0~11)
                    var hours12 = date.getHours() % 12;
                    replacement = hours12;
                    break;
                case _this.patterns.PATTERN_ZONE_NAME: //z
                    replacement = _this.getZoneNameValue(date)['name'];
                    break;
                case _this.patterns.PATTERN_ZONE_VALUE: //Z
                    replacement = _this.getZoneNameValue(date)['value'];
                    break;
                case _this.patterns.PATTERN_ISO_DAY_OF_WEEK: //u
                    break;
                case _this.patterns.PATTERN_ISO_ZONE: //X
                    break;
                default:
                    break;
            }
